...
GCode2DViewer.preview = function (gcodeStr, colors, canvas) {
    ...
    if (gcode.size.max.x === gcode.size.min.x && gcode.size.max.y === gcode.size.min.y) {
        return
    }
    ...
    var i = 0,
        ratio = calculateRatio(gcode, canvas)
    ...
    for (i = 0; i < gcode.lines.length; i++) {
        ...
        } else if ((gcode.lines[i].type === 'G2' || gcode.lines[i].type === 'G3') && colors.G2G3 !== undefined) {
